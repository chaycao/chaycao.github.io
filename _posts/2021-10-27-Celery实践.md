...
笔者在近期工作中有接触到 Celery，这是一个开源的分布式任务队列（Distributed Task Queue），在 Github 上现有 18k star，主要可以用于实现应用中的异步任务和定时任务。笔者写下此文总结对 Celery 的了解和在工作中的使用。本文的大概内容如下：
...
`Task` 已经定义完成，需要发起异步任务，可通过 `Task` 的 `delay` 方法将消息发送给队列，例如在用户注册完成时，发起异步发邮件的任务：
...
## Celery Beat + Django ORM 实现可配置的定时任务
...
以上便是笔者在工作中接触到 Celery 所收获的内容，如果需要实现异步任务、定时任务，可以考虑使用 Celery。
