...
下面重点了解下```LOCK_IF_MP```和LOCK前缀就行了，其他我也不懂
- Linux的x86实现：
```
...
下面通过一个例子来说明ABA问题的隐患：
![ABA-1.png](http://upload-images.jianshu.io/upload_images/2489662-fdb70dd479460ab0.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
现有一个用单向链表实现的堆栈，栈顶为A，这时线程T1已经知道A.next为B，然后希望用CAS将栈顶替换为B：
```java 
head.compareAndSet(A,B);
```
在T1执行上面这条指令之前，线程T2介入，将A、B出栈，再push D、C、A，此时堆栈结构如下图，而对象B此时处于游离状态：
![ABA-2.png](http://upload-images.jianshu.io/upload_images/2489662-0e5164d5548d4063.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
此时轮到线程T1执行CAS操作，检测发现栈顶仍为A，所以CAS成功，栈顶变为B，但实际上B.next为null，所以此时的情况变为：
![ABA-3.png](http://upload-images.jianshu.io/upload_images/2489662-5ad6b2168340beee.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
...
3. [JAVA-CAS简介以及ABA问题
](http://java.jr-jr.com/2015/12/02/java-CAS/#ABA_u95EE_u9898)
